...
public class Spawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(timer > max_time){ //Se o tempo limite do "nível" for atingido, um novo objeto é criado
            ...
            max_time = 0,9*max_time;//Diminui o intervalo de tempo conforme novos obstáculos forem gerados
            if(max_time<max_time_0/2){
                tag++; //Se o novo max_timem for menor que metade (ajuste) do original, um novo obstáculo passa a surgir
            }
            pontuacao += pontos;
