...
\lstset{
extendedchars=true,
literate={á}{{\'a}}1 {ã}{{\~a}}1 {é}{{\'e}}1 {ú}{{\'u}}1,
language=csh,
...
\chapter{Estado do aplicativo}
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \node[rect, rounded corners, right of=player, node distance=6cm](UE) {User\_Experience};
            \node[rect, rounded corners, below of=player, node distance=1.5cm](GH) { Game\_Handler };
            \path[line](player) -- (UE);
            \path[line](player) -- (GH);
            
            \node[elli, right of=UE, node distance=4cm](mc) {Main Camera};
            ...
            \node[rect, rounded corners, left of=GH, node distance=4cm](GM) {GameManager};
            \node[rect, rounded corners, below of=GM, node distance=1cm](sh) {spawn\_handler};
    ...
    Além dos elementos mostrados acima, temos também o player e o objeto "EventSystem".
